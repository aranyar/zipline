 ...
 * JS context, so it must not be reachable from the network. On iOS the simulator shares
 * the host network, so the dev tools connect via 127.0.0.1 directly. */
internal actual class DebugServerSocket actual constructor(port: Int) {
  ...
  init {
    ignoreSigPipe()
    fd = memScoped {
      // Resolve "127.0.0.1" via getaddrinfo instead of filling sockaddr_in by
      // hand: sockaddr_in field types differ in width across platforms (e.g.
      // sin_family is 8-bit on Darwin, 16-bit on Linux) and cannot be used
      // from the shared nativeMain source set.
...
internal actual fun connectDebugSocket(host: String, port: Int): DebugSocket =
  DebugSocket(connectTcp(host, port.toString()))
