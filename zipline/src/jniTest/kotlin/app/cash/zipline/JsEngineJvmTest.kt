...
class HermesJvmTest {
  ...
  @Test fun exceptionsInScriptIncludeStackTrace() {
    ...
    // JsException.addJavaScriptStack strips the column number that
    // Hermes JNI bridge includes in stack frames.
    assertEquals("JavaScript.f2(test.js:8)", t.stackTrace[0].toString())
    assertEquals("JavaScript.f1(test.js:4)", t.stackTrace[1].toString())
    assertEquals("JavaScript.global(test.js:1)", t.stackTrace[2].toString())
  ...
  @Test fun dateNow() {
    ...
    Thread.sleep(100)
    val nowMillis = js.evaluate("Date.now()") as Double
    Thread.sleep(100)
